#!/bin/bash
# Sakura AI Docker 安装/升级脚本 (Debian Linux)
# 
# 使用方法：
...
install() {
    ...
    # 执行数据库迁移
    log_info "📊 执行数据库迁移..."
    if docker compose -f "$COMPOSE_FILE" exec -T sakura-ai npx prisma migrate deploy; then
        log_success "数据库迁移完成"
    else
        log_warning "数据库迁移未完成，可能是首次启动数据库尚未就绪"
        echo ""
        echo "请稍后手动执行迁移命令："
        echo "  docker compose -f $COMPOSE_FILE exec sakura-ai npx prisma migrate deploy"
        echo ""
    fi
...
# 显示帮助
help() {
    echo "Sakura AI Docker 管理脚本 (Debian Linux)"
    echo ""
    echo "用法: $0 <命令> [参数]"
    echo ""
    echo "命令:"
